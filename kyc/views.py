    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if not user.passport_photo_url:
                ...
                messages.error(self.request, "Passport size photo not found.")

                return redirect("kyc:kyc_verify")

            live_capture = form.cleaned_data.get("live_capture")

            if not live_capture:

                messages.error(self.request, "Live capture image missing.")

                return redirect("kyc_verify")

            format, imgstr = live_capture.split(";base64,")

            ext = format.split("/")[-1]

            selfie_file = ContentFile(
                base64.b64decode(imgstr), name=f"live_selfie.{ext}"
            )
            ...
            passport_path = user.passport_photo_url.path

            selfie_path = kyc.selfie_image.path
            ...
            if verified:
                ...
                user.save()
                messages.error(
                    self.request, "Face Verification Failed. Please try again."
                )
...
class KYCResultView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        context = {
            ...
            "passport_photo": request.session.get("passport_photo"),
            "selfie": request.session.get("selfie"),
        }
