...
class ApplyLoanView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        application.add_status_history('submitted', self.request.user,'Application submitted')
...
class ApplicationDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        ...
        return ( user == application.applicant or user == application.officer or user.role =='admin')
    
    def handle_no_permission(self):
        messages.error(self.request, 'You dont have permission to view this application')
        return redirect('accounts:dashboard')
...
class UploadDocumentsView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        file_name = f"{application.id}_{form.instance.document_type}_{uploaded_file.name}"
        ...
        # When additional documents are provided after an info request,
        # reflect that in the application status history so the officer
        # can clearly see the loop from the flowchart.
