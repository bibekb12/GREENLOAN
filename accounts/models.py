...
"""Base user manager for the override of the username to all applications."""
...
class User(AbstractUser):
    ...
    role = models.CharField(
        _("applicant role"), max_length=50, choices=ROLE_CHOICES, default="customer"
    )
    ...
    gender = models.CharField(
        _("gender of the applicant"), max_length=50, choices=GENDER_CHOICES, blank=True
    )
    nationality = models.CharField(
        _("applicant nation"), max_length=50, default="Nepali"
    )
    ...
    occupation = models.CharField(
        _("applicant occupation"),
        max_length=50,
        choices=APPLICANT_OCCUPATION_CHOICES,
        blank=True,
    )
    employer_name = models.CharField(
        _("applicant employer if any"), max_length=100, blank=True
    )
    monthly_income = models.DecimalField(
        _("applicant income"), max_digits=15, decimal_places=2, null=True, blank=True
    )
    citizenship_number = models.CharField(
        _("applicant citizenship number"), max_length=50, blank=True
    )
    national_id_number = models.CharField(
        _("applicant national id number"), max_length=50, blank=True
    )
    ...
    # documents
    citizenship_front_url = models.FileField(upload_to="kyc", blank=True)
    citizenship_back_url = models.FileField(upload_to="kyc", blank=True)
    passport_photo_url = models.FileField(upload_to="kyc", blank=True)
